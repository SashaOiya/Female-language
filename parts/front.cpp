...
int main ( int argc, char *argv[] )
{
    struct Language_t language = {};
                                               // errors
    if ( Language_Ctor ( &language, ( argc > 0 ) ? argv[1] : nullptr ) != NO_ERR ) {
        return EXIT_FAILURE;
    }
$
...
Errors_t Language_Ctor ( struct Language_t *language, const char *input_file_name ) // wtf baaad   + dtor
{
...
$
    Errors_t ret_err = File_Reader ( &language->file, input_file_name );
    if ( ret_err != OK_FILE ) {
...
Errors_t File_Reader ( struct File_t *file, const char *input_file_name )  // +
{
    ...
    if ( ret_code != file->file_size  ) {
        if ( feof ( input_f ) ) {
            printf ( "Error reading from text file: unexpected end of file. Function \"File_reader\"." 
                     "ret_code = %d, file_size = %d \n", ret_code, file->file_size );
        }
...
char *File_Skip_Spaces ( char *data, const int file_size )   // +
{
    assert ( data != nullptr );
    assert ( file_size != 0  );
