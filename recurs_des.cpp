...
Node_t *Get_Statement_List ( struct Position_t *position )
{
    ...
    struct Node_t *new_node = nullptr; //Create_Node ( NODE_TYPE_OP, OP_CONNECT, nullptr, nullptr );
    ...
    if ( position->data[position->index].type == NODE_TYPE_OP &&
        ...
        while ( position->data[position->index].type == NODE_TYPE_IF ||
            ...
            //    new_node->right = Create_Node ( NODE_TYPE_OP, OP_CONNECT, state_node, nullptr );
            //}
            //new_node->right = state_node;
...
Node_t *Get_Statement ( struct Position_t *position, struct Node_t *node )
{
    ...
    //node->left = new_node;
    //if ( new_node != nullptr ) {
    //node->right = Create_Node ( NODE_TYPE_OP, OP_CONNECT, nullptr, nullptr );
