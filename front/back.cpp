...
$       fprintf ( language->out_file, "push %d;\n", tree_node->value );
    ...
    else if ( tree_node->type == NODE_TYPE_OP && tree_node->value == OP_SUB ) {
        Back_End_Body ( language, tree_node->right );
        Back_End_Body ( language, tree_node->left  );
        language->id += 1;
$       fprintf ( language->out_file, "%s;\n", Get_Op_Asm_Code ( tree_node->value ) );
        ...
        Back_End_Body ( language, tree_node->right );
$       fprintf ( language->out_file, "pop [%d];\n", language->name_cell[tree_node->left->value].name_code );
        language->id += 2;
...
$       else {
    ...
    else if ( tree_node->type == NODE_TYPE_INPUT ) {
        fprintf ( language->out_file, "in;\n", language->id );
        ++(language->id);
        return ;
    }
    else if ( tree_node->type == NODE_TYPE_OUTPUT ) {
        fprintf ( language->out_file, "out;\n", language->id );
        ++(language->id);
        return ;
    }
